          ...
          <div class="card-body" v-if="Read == 1">
             ...
             <table-column
              ...
              >
                <template slot-scope="row">
                  <div>
                    <a href="#"  @click="$refs.prescription_info.open(),prescriptionModel(row.prescription_path)" v-b-popover.hover ="'Open Priscription'">

                   <img @error="onImageLoadFailure($event)"  height="100" :src="mobileImageLink + row.prescription_path"/>
                    </a>
            ...
            <sweet-modal  ref="prescription_info" modal-theme="dark" overlay-theme="dark" width="70%">
              <div class="card-body prescription">
                <img @error="onImageLoadFailure($event)" height="500" :src="mobileImageLink + Prescription"/>
...
import { mobileImageLink , imageLink} from '../../../helpers/constants' 
...
export default {
  ...
  methods: {
      async downloadLink(file){
         window.location.href =  mobileImageLink + file;
    },
    ...
    async getUserPrescription({ page, filter, sort }) {
      try {
        ...
        const response = await axios.get(
          `/api/admin/users/userprescription?user_id=${userId}&page=${page}&filter=${filter}`
        );
