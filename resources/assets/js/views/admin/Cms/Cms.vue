  ...
  <div class="main-content">
    ...
    <div class="row">
      <div class="col-sm-12 hide">
        <div class="card">
          <div class="card-body">
            <table-component v-if="Read == 1" ref="allUsers" :data="getCms" sort-by="row.id" sort-order="desc" table-class="table">

              <table-column show="title_english" label="Title"/>
              <table-column show="slug" label="Slug"/>
              <table-column :sortable="false" :filterable="false" label="Keywords">
                <template slot-scope="row">
                  <div v-html="$options.filters.tagged(row.keywords)"/>
                </template>
              </table-column>

              <table-column label="status">
                <template slot-scope="row">
                  <div v-if="row.status == 1">Active</div>
              ...
              <table-column label="Description">
                <template slot-scope="row">
                  <div class="cms-description">{{ row.description }}</div>
                </template>
              </table-column>
...
export default {
  ...
  methods: {
    ...
    async getCms({ page, filter, sort }) {
      try {
        const response = await axios.get(
          `/api/auth/get?page=${page}&filter=${filter}`
        );
        return {
    ...
    async destroyCmsData(id) {
      try {
        let response = await window.axios.delete("/api/auth/" + id);
